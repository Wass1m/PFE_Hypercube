...
export default function TestPrincipal_fourmis() {
  ...
  const handleSpinner = () => {
    setFormData({ ...formData, loading: "block" });
  };
  ...
  const execEuler = () => {
    ...
    console.log(
      "Longueur de la solution Euler =" + heuristique.getTour().length
    );
    console.log("");
    console.log("-----------Evaluation - niv 1---------");
    var niv = 1;
  ...
  const execEval1 = () => {
    var mC = prompt(
      "Veuillez donner la métrique du cycle eulérien correspondante",
      "métrique"
    );
    var mcx = parseFloat(mC);
    console.log(mcx);

    if (heuristiqueE == null) {
      alert("Veillez lancer l'heuristique d'euler d'abord ");
    } else {
      var niv = prompt(
        "Veuillez préciser le niveau de communication",
        "niveau"
      );
      ...
      console.log("");
      console.log(
        "****Evaluation 1 : Heuristique d'Euler vs Cycle Eulérien***"
      );
      var evaluation = new Eval(hgraph, heuristiqueE.getTour(), niv);
      evaluation.calculTableRoutage();
      evaluation.run(1);
  ...
  const onSubmit_Eval1 = (e) => {
    e.preventDefault();
    execEval1();
  };

  const onSubmit_Eval2 = (e) => {
    e.preventDefault();
    execEval2();
  };

  const onSubmit_Euler = (e) => {
    e.preventDefault();
    execEuler();
  };

  const onSubmit_ACO = (e) => {
    e.preventDefault();
    execACO();
  };
  ...
  var AntColony = (function () {
    ...
    AntColony.prototype.algo = function () {
      ...
      for (var t = 0; t < this._maxIterations; t++) {
        ...
        console.log("nombre d'arc = " + tabEuler.length);
        console.log("taille colonie = " + this._colonySize);
        this._createAnts();
        this._iterationBest = null;
        for (var n = 0; n < this._colonySize; n++) {
          //pour chaque fourmi
          console.log("fourmis id :  " + n);
          ...
          console.log(
            " la fourmis " +
              n +
              " a construit une solution de longueur  " +
  ...
  var Ant = (function () {
    ...
    Ant.prototype.addPheromone = function () {
      ...
      for (var index in this._tour) {
        this._graphA
          .getEdges()
          [this._tour[index].getId()].addPheromone(
            Math.sqrt(this._tour.length)
          );
      }
    };
    return Ant;
            ...
            if (x != -1) {
              for (var j = noeudCourant.getCPT(); j >= 2; j--) {
                ...
                noeudCourant.addEtiq(
                  c,
                  noeudCourant.getTableEdges().length +
                    j -
                    noeudCourant.getCPT()
                );
              }
      ...
      <main>
          ...
          <Container>
            ...
            <div className={classes.heroButtons}>
              <Grid container spacing={2} justify="center">
                <Grid item>
                  <Linkus style={{ textDecoration: "none" }} to="/">
        ...
        <Container className={classes.cardGrid} maxWidth="lg">
          <div className={classes.divMargin}>
            <form className={classes.customForm} onSubmit={(e) => onSubmit(e)}>
            ...
            >
              ...
              <div className="form-group">
                <input
                  ...
                  type="number"
                  value={initPheromone}
                  placeholder="Pheromone initiale"
                  name="initPheromone"
                  required
                  onChange={(e) => onChange(e)}
                />
          ...
          <div className={classes.divMargin}>
            <h1>Graphe adjoint crée, nombre de noeuds :</h1>
            <h1>{nbrNA > 0 ? nbrNA : null}</h1>

            <h1>Nombre d'aretes : </h1>
            <h1>{nbrEA > 0 ? nbrEA : null}</h1>
            <br />
            <br />
            <h1 style={{ display: aco_loading }}>longueur de la sol ACO : </h1>
            <h2 style={{ display: aco_loading }}>
              {longueurACO > 0 ? longueurACO : null}
            </h2>

            <h1 style={{ display: aco_loading }}>
              métrique pour ACO niv 1 est :{" "}
            </h1>
            <h2 style={{ display: aco_loading }}>
              {evaACO1 > 0 ? evaACO1 : null}
            </h2>
            <h1 style={{ display: aco_loading }}>
              métrique pour ACO niv {niveau} est :{" "}
            </h1>
            <h2 style={{ display: aco_loading }}>
              {valueEnd1 > 0 ? valueEnd1 : null}
            </h2>
            <img
              id="spinner"
              src={spinner}
          ...
          <div className={classes.divMargin}>
            <div style={{ padding: "20%", display: aco_loading }}>
              <h1 style={{ textAlign: "center" }}>
                {heur1} {""} {niveau}
              </h1>
              <ProgressProvider valueStart={0} valueEnd={valueEnd1}>
                {(value) => (
                  <CircularProgressbar
            ...
            <div style={{ padding: "20%", display: euler_loading }}>
              <h1 style={{ textAlign: "center" }}>
                {heur2} {""} {niveaueul}
              </h1>
              <ProgressProvider valueStart={0} valueEnd={valueEnd_euler}>
                {(value) => (
                  <CircularProgressbar
            ...
            <div style={{ padding: "20%", display: eval_loading }}>
              <h1 style={{ textAlign: "center" }}>Amelioration</h1>
              <ProgressProvider valueStart={0} valueEnd={amelioration}>
                {(value) => (
                  <CircularProgressbar
